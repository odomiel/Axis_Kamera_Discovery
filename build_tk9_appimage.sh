...
mkdir -p "$APPDIR/app"
cp "$ROOT/axis_discovery_gui.py" "$ROOT/axis_discovery_cli.py" "$ROOT/README.md" "$APPDIR/app/"
...
OUT="$ROOT/AxisDiscovery-tk9-${VERSION}-x86_64.AppImage"
ARCH=x86_64 "$AIT" --appimage-extract-and-run "$APPDIR" "$OUT" 2>&1 | tail -5
ln -sfn "$(basename "$OUT")" "$ROOT/AxisDiscovery-tk9-x86_64.AppImage"
